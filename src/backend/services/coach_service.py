...
from models.schema import CaseItem, CoachResponse, SynthesisOutput
...
class CoachService:
    ...
    async def _call_llm_synthesis(self, query: str, cases: List[Dict[str, Any]]) -> Optional[SynthesisOutput]:
        ...
        system_prompt = (
            "You are a professional counselor seeking guidance on how to help a client. You must use ONLY the content from the provided case responses.\n"
            "Write a clear, professional reply (150-250 words) with evidence-based guidance, organized in short paragraphs.\n"
            "Rules:\n"
            ...
            "3) Provide professional guidance on assessment, intervention strategies, and client support approaches.\n"
            "4) Focus on evidence-based practices and professional considerations.\n"
        ...
        user_prompt = (
            f"Counselor query about client:\n{query}\n\n"
            f"Professional case examples (use only this material, and cite precise sentences by sent_id):\n{cases_str}\n\n"
            "Write professional guidance with inline [Ccase:Sent] citations, then return the JSON."
        )
    ...
    def _light_verify(self, synthesis: SynthesisOutput, cases: List[Dict[str, Any]]) -> Tuple[bool, str]:
        ...
        for ref in synthesis.citations:
            try:
                ...
                if not isinstance(ref, dict) or "case_id" not in ref or "sent_id" not in ref:
                    logger.warning(f"Invalid citation format in verification: {ref}")
                    continue
                ...
                if not case:
                    logger.warning(f"Verification failed: Bad citation: unknown case {case_id}")
                    return False, f"Bad citation: unknown case {case_id}"
                
                # If sentence index is out of range, clamp it to the valid range
                if sent_id < 0:
                    logger.warning(f"Fixing negative sentence index: case {case_id} sent {sent_id} -> 0")
                    ref["sent_id"] = 0
                elif sent_id >= len(case.response_sentences):
                    valid_idx = len(case.response_sentences) - 1
                    logger.warning(f"Fixing out-of-range sentence index: case {case_id} sent {sent_id} -> {valid_idx}")
    ...
    async def coach(self, query: str, cases: List[Dict[str, Any]]) -> CoachResponse:
        ...
        for ref in synthesis.citations:
            try:
                # Validate citation format
                if not isinstance(ref, dict) or "case_id" not in ref or "sent_id" not in ref:
                    logger.warning(f"Invalid citation format: {ref}")
                    continue
                ...
                if not case.response_sentences or sent_id < 0 or sent_id >= len(case.response_sentences):
                    logger.warning(f"Sentence index out of range: case_id={case_id}, sent_id={sent_id}, available_sentences={len(case.response_sentences) if case.response_sentences else 0}")
                    continue
                
                # Get the sentence
                sent = case.response_sentences[sent_id]
                citations.append({
                    "case_id": case_id,
                    "sent_id": sent_id,
                    "start": sent.start,
                ...
                logger.info(f"Successfully built citation for case {case_id}, sentence {sent_id}")
                
            except Exception as e:
                logger.error(f"Error building citation for {ref}: {e}")
                continue
