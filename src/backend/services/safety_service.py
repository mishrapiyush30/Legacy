...
class SafetyService:
    ...
    def crisis_severity(self, text: str) -> str:
        """
        Determine crisis severity for counselor context - allow professional discussion.
        Returns: 'none' | 'mild' | 'moderate' | 'high'
        """
        t = text.lower()
        # For counselors, crisis language is professional discussion, not patient crisis
        # Only block if it's clearly a personal crisis from the counselor
        personal_crisis = re.search(r"\b(i\s+(am|feel|want|need)|my\s+(client|patient))\b", t)
        if personal_crisis and re.search(r"\b(suicid(e|al)|kill myself|end my life|want to die)\b", t):
            return "high"
        return "none"
