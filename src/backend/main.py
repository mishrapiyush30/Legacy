...
def main():
    ...
    parser = argparse.ArgumentParser(description="Compass - Safety-First Coaching")
    ...
    # Run the FastAPI app
    logger.info(f"Starting Compass on {args.host}:{args.port}")
    uvicorn.run(
        "backend.api.app_new:app",
        host=args.host,
        port=args.port,
        reload=args.reload
    )
