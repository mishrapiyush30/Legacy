...
const CoachPanel = ({ response, isLoading, onCoach, hasSearchResults }) => {
  ...
  const formatResponse = () => {
      ...
      return (
        <div className="space-y-4">
          ...
          {response.citations && response.citations.length > 0 && (
            <div className="mt-4">
              <h3 className="text-sm font-medium text-gray-700 mb-2">Professional References:</h3>
              <div className="flex flex-wrap gap-2">
                {response.citations.map((citation, index) => (
                  ...
                  >
                    ...
                    <span>Reference #{citation.sent_id + 1}</span>
                  </div>
                ))}
              </div>
            </div>
    ...
    // Fallback to simple text if no markdown answer
    return (
      <div className="space-y-4">
        <p className="text-sm text-gray-800">No structured response available.</p>
      </div>
    );
  };
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-teal-700">Professional Guidance</h2>
        <button 
          onClick={onCoach}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-8 text-gray-500">
          <p>Search for cases to get professional guidance</p>
        </div>
      ) : (
        formatResponse()
      )}
